...
the second one is the **opinionated** pre-config. Like many other devs, I have my own preferences for how I want to use my tools. The pre-config of **Neovim** (btw) and recently **tmux** is very annoying. Binding a key to **btop** also doesn't make a lot of sense to me. And mapping _CapsLock_ to the _xcompose_ key instead of _Esc_ is just a crime to any Vim user.
...
I've been using Omarchy since August 2025, and it has been improving a lot since then. But I think at some point, when the user base grows, DHH should think about having _two_ versions of Omarchy (just like 3b1b's [Manim](https://github.com/3b1b/manim)): a base/community version that is more minimal and is maintained by the community, and an opinionated version like the current one.
