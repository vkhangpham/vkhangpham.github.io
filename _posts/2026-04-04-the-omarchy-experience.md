...
but wait, isn't being able to configure and customize your own machine the point of using Linux in general and Arch in particular? Well I think DHH understands the spirit too, so Omarchy doesn't take away the fun from you; instead, it makes the experience easier and simpler -- which is exactly what I want. Man, I can never forget the headache that **Hyprland** gave me when I was trying to set it up in college. <img src="/assets/emotes/thevoices.webp" alt="thevoices" class="inline-emote">
...
the second one is the **opinionated** pre-config. Like many other devs, I have my own preferences for how I want to use my tools. The pre-config of **Neovim** (btw <img src="/assets/emotes/hackermans.webp" alt="hackermans" class="inline-emote">) and recently **tmux** is very annoying. Binding a key to **btop** also doesn't make a lot of sense to me. And mapping _CapsLock_ to the _xcompose_ key instead of _Esc_ is just a crime to any Vim user. Luckily, we already have tools like [chezmoi](https://www.chezmoi.io/) and [GNU Stow](https://www.gnu.org/software/stow/) to the rescue!
